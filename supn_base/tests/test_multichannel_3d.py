...
import torch
from testing_utils import make_3d_multi_channel_supn_weights, get_test_data
...
supn_prec_cholesky = make_3d_multi_channel_supn_weights(local_connection_dist, im_size_d, im_size_w, im_size_h, num_ch, RANDOM_SEED, num_supn_models)
...
data = get_test_data(num_data_batches, num_supn_models, im_size_w, im_size_h, random_seed=RANDOM_DATA_SEED, requires_grad=False, num_ch=num_ch, im_size_d=im_size_d)
...
def test_samples():
    ...
    assert(samples.shape == (num_data_batches, num_supn_models, num_ch, im_size_d, im_size_w, im_size_h))
